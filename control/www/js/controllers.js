...
.controller('HomeCtrl', function($scope, $sce, $http, $ionicPopup, $state, $ionicLoading, $ionicModal, serveLogin) {
	$scope.loginData = {};
	if(serveLogin.isLogin()){
		$scope.accountIsVisible = false;
		$scope.pageIsVisible = true;
			...
			.then(
				...
				function (){
					if($scope.trying == 0){
						...
						$state.go('tab.descargas', {}, {reload: true});
					}
...
.controller('StudentCtrl', function($scope, $sce, $http, $ionicPopup, $state, $ionicLoading, $ionicModal, $timeout, productService, serveLogin) {
	...
	$scope.personScore = {id_curso : $scope.Class[0]};
	$scope.callScorePerson = function(){
		if(serveLogin.isLogin()){
			console.log($scope.personScore);
			$scope.show($ionicLoading);
			var urlCompleta ="http://www.vocabulario.esy.es/getScoreClass.php";
			var postUrl = $sce.trustAsResourceUrl(urlCompleta);
...
.controller('ChallengeCtrl', function($scope, $sce, $http, $ionicPopup, $state, $ionicLoading, $ionicModal, $timeout, serveLogin, productService) {
	console.log("ChallengeCtrl");
	$scope.user = serveLogin.getUser();
	$scope.Class = productService.getClass();
	$scope.Challenge = productService.getChallenge();
	$scope.getChallenges = function() {
		console.log('Doing Edit person', $scope.editPersonData);
		var entity = $scope.user;
		var objJSON = {id_curso: $scope.Class[0], id_desafio: $scope.Challenge[0]};
		// Do the call to a service using $http or directly do the call here
		var urlCompleta ="http://www.vocabulario.esy.es/getScoreChallengeService.php";
		var postUrl = $sce.trustAsResourceUrl(urlCompleta);
		console.log(objJSON);
		$http.post(postUrl, objJSON)
		.then(
...
.controller('ChallengesListCtrl', function($scope, $sce, $http, $ionicPopup, $state, $ionicLoading, $ionicModal, $timeout, serveLogin, productService) {
	console.log("ChallengeCtrl");
	$scope.user = serveLogin.getUser();
	...
	$scope.callChallenges = function(){
		...
		if(serveLogin.isLogin()){
			console.log($scope.Class);
			console.log($scope.Class[0]);
			$scope.course = {id_curso:$scope.Class[0]};
			var entity = $scope.user;
	...
	$scope.setChallenge = function(id, name) {
		productService.addChallenge(id, name);
	};
